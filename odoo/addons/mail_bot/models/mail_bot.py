...
class MailBot(models.AbstractModel):
    ...
    def _ask_local_ai(self, prompt, user):
          try:
              response = requests.post(
                  "http://127.0.0.1:1234/v1/chat/completions",
                  headers={"Content-Type": "application/json"},
                  json={
                      "model": "mistral-7b-instruct",  # Replace with your actual model name
                      "messages": [
                          
                           {
                        "role": "system",
                        "content": f"You are a helpful assistant answering questions for a user named {user.name} (ID: {user.id}, Email: {user.email})."
                           },
                            {
                        "role": "user",
                        "content": prompt
              ...
              return f"[Erreur IA locale] {str(e)}" 
               

    def _get_answer1(self, record, body, values, command=False):
        ...
        if record.channel_type == "chat" and odoobot in record.channel_member_ids.partner_id:
            ...
            if odoobot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                ...
                return html_escape(
                    _("Great! 👍%(new_line)sTo access special commands, %(bold_start)sstart your sentence "
                      "with%(bold_end)s %(command_start)s/%(command_end)s. Try getting help.")) % self._get_style_dict()
            ...
            elif "who am i" in body.lower() or "my info" in body.lower() or "who i am" in body.lower() :
                 user = self.env.user
                 groups = ', '.join([group.name for group in user.groups_id])
                 return html_escape(    
                                     _(
                    "Here is your information User ID : %(uid)s:<br/><br/>"
                    "<b>User ID:</b> %(uid)s<br/>"
                    "<b>Name:</b> %(name)s<br/>"
                    ...
                    'groups': groups or 'No roles assigned'
                }   
                     
                 )
            elif self._is_help_requested(body) or odoobot_state == 'idle':
                return html_escape(
                    _("Nous sommes Sofabi, en train de développer un bot qui vous aide dans votre travail et vous facilite les tâches, "
                ...
                self.env.user.odoobot_failed = False
                user = self.env.user
                ai_response = self._ask_local_ai(body, user)
                return html_escape(ai_response)
    
    def _get_answer(self, record, body, values, command=False):
        user = self.env.user
        
        # Special command to return user info
        if "who am i" in body.lower() or "my info" in body.lower() or "who i am" in body.lower():
            groups = ', '.join([group.name for group in user.groups_id])
            return html_escape(_(
            ...
            # Default: forward question to local AI
        ai_response = self._ask_local_ai(body, user)
        return html_escape(ai_response)
